	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' class="select" v-if="type!='info'"  label="心理等级" prop="xinlidengji">
					<el-select :disabled="ro.xinlidengji" v-model="ruleForm.xinlidengji" placeholder="请选择心理等级" >
						<el-option
							v-for="(item,index) in xinlidengjiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0","display":"inline-block"}' v-else class="input" label="心理等级" prop="xinlidengji">
					<el-input v-model="ruleForm.xinlidengji"
						placeholder="心理等级" readonly></el-input>
				</el-form-item>
	...
	data() {
		...
		return {
			...
			rules: {
				...
				xinlidengji: [
					{ required: true, message: '心理等级不能为空', trigger: 'blur' },
				],
				xinlipinggu: [
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='xinlidengji'){
							this.ruleForm.xinlidengji = obj[o];
							this.ro.xinlidengji = true;
							continue;
						}
						if(o=='xinlipinggu'){
            ...
            this.xinlidengjiOptions = "普通人,轻度患者,中度患者,重度患者".split(',')
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xinlidangan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xinlidanganCrossAddOrUpdateFlag = false;
						 this.parent.search();
